...
// ConectBlue: ask our own site whether there is a newer ConectBlue build,
// instead of asking the official RustDesk API (which would offer the
// vanilla unbranded installer). SOFTWARE_UPDATE_URL is set to our own
// GitHub release tag page, matching the "tag"/"download" URL shape the
// rest of the update flow (updater.rs, flutter_ffi.rs's download-file-*
// handler) already expects -- only the owner/repo in the allowlist was
// changed to match (see get_update_download_file_from_url in updater.rs).
const CONECTBLUE_RELEASE_TAG_URL: &str =
    "https://github.com/conectblue/rustdesk/releases/tag/nightly";

#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let client = reqwest::Client::new();
    let resp = client
        .get("https://conectblue.com.br/versao.json")
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await?;
    let bytes = resp.bytes().await?;
    let json: serde_json::Value = serde_json::from_slice(&bytes)?;
    let latest_build = json.get("build").and_then(|v| v.as_str()).unwrap_or("");
    let current_build = env!("CONECTBLUE_BUILD");

    if !latest_build.is_empty() && latest_build != current_build {
        let response_url = CONECTBLUE_RELEASE_TAG_URL.to_string();
        #[cfg(feature = "flutter")]
        {
